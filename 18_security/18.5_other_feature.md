...
Docker 預設啟用了 Seccomp，並使用預置的 [預設設定檔案](https://docs.docker.com/engine/security/seccomp/) 作為 allowlist：預設拒絕未顯式允許的系統呼叫，並額外允許常見應用所需的呼叫。Docker 官方文件將其描述為預設停用約 44 個系統呼叫（核心與 Docker 版本不同會有差異），例如與核心模組、系統重啟或特權命名空間操作相關的呼叫。

如果你對應用的系統呼叫特徵瞭如指掌，你可以為容器定製專屬規則。

#### 實戰：停用 chmod 系統呼叫過濾

> **務必先讀**：`--security-opt seccomp=<file>` 是**整體替換**預設 profile，不是在預設之上疊加。
> 如果自己寫一個 `"defaultAction": "SCMP_ACT_ALLOW"` 的 blocklist，等於把上面那份
> deny-by-default 的官方 allowlist 整個丟掉——你擋住了 `chmod`，卻把預設被攔下的那
> ~44 個高危呼叫（`unshare`、`keyctl`、`add_key`、`bpf`、`perf_event_open`、
> `init_module`、`reboot` 等）全部放行，**淨效果比不加這個引數還危險**。
> 正確做法是**以官方預設 profile 為基線派生**。

先取官方預設 profile，再在它之上把 `chmod` 改成拒絕：

```bash
$ curl -fsSL -o default.json \
  https://raw.githubusercontent.com/moby/moby/master/profiles/seccomp/default.json

## 在 default.json 中找到包含 "chmod" 的那條 syscalls 記錄，把它從 allow 清單中移除；
## 或追加一條更靠前的顯式拒絕規則：
...
```
應用只要被劫持進行越界嘗試，其作業系統層指令便會立刻吃癟——同時預設 profile 原有的
~44 項攔截仍然生效。
