...
預設的樣板檔案名稱為 `compose.yaml` (也相容 `docker-compose.yml` 等歷史檔案名)，格式為 YAML。

本書同時維護一份可直接執行 `docker compose config` 校驗的[完整 Compose 範例](../examples/validated/compose/compose.yaml)。下面的圍欄與該檔案由測試強制保持一致；需要複製完整檔案時，以任一處為準。
...
指定 `Dockerfile` 所在資料夾的路徑 (可以是絕對路徑，或者相對 Compose 檔案的路徑)。`Compose` 將會利用它自動建立這個映象，然後使用這個映象。
...
`configs` 來自 Compose Specification。它在 Swarm 中是原生物件；在本地 `docker compose` 模式下通常以檔案掛載的形式實現，具體能力取決於 Compose 版本與執行平台。
...
```
允許容器中執行一些特權指令。`privileged: true` 會顯著擴大容器對宿主機裝置和 Linux capability 的訪問範圍，生產環境應優先使用更窄的 `cap_add`、`devices` 或只讀掛載；更多說明見 [核心能力機制](../18_security/18.4_kernel_capability.md)。

```yaml
privileged: true
```
指定容器退出後的重啟策略。該指令對保持服務始終執行十分有效，在生產環境中推薦設定為 `always` 或者 `unless-stopped`。
...
如果在指令行裡顯式指定某個帶 profile 的服務，Compose 會自動啟動這個目標服務及其宣告的依賴；但不會自動啟動同一 profile 下的其它服務，除非也顯式指定或透過 `--profile` / `COMPOSE_PROFILES` 啟用。
