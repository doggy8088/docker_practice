...
Docker Compose 提供了豐富的指令來管理專案和容器。本節將詳細介紹這些指令的使用格式和常用選項。

### 何時用哪個指令：場景化指南

在學習具體指令前，讓我們從使用場景出發，這樣可以幫助你更快地找到需要的指令：

**專案啟動與停止**：

- `docker compose up`：第一次啟動專案，拉取映象、建立容器
...
- `docker compose exec`：進入正在執行的容器執行指令

**建立與更新**：

- `docker compose build`：重新建立映象（修改 Dockerfile 後）
- `docker compose pull`：更新所有映象到最新版本
...
* `-f, --file FILE` 指定使用的 Compose 樣板檔案。預設會自動識別 `compose.yaml`（也相容 `docker-compose.yml` 等），並且可以多次指定。

* `-p, --project-name NAME` 指定專案名稱，預設將使用所在目錄名稱作為專案名。

* `--verbose` 輸出更多除錯訊息。（Compose V2 中該選項仍然生效，但已從 `--help` 中隱藏，不再是推薦用法；請改用 `docker --log-level debug compose ...`。）
...
服務容器一旦建立後，將會帶上一個標記名，例如對於 web 專案中的一個 db 服務，建立出的映象是 `web-db`。（Compose V1 用下劃線拼接為 `web_db`，V2 起統一改為連字元，只有加 `--compatibility` 才會回到下劃線。）
...
當前 Compose CLI 仍支援 `docker compose scale`。實際使用中，更常見也更便於和建立/重建流程放在一起的寫法是透過 `docker compose up --scale` 完成。
...
> **說明**：如果 Compose 檔案為服務指定了 `container_name`，該服務無法擴充套件到多個容器。需要擴縮容的服務應使用 Compose 自動生成的容器名，並透過服務名做 DNS 訪問。
...
預設情況，如果服務容器已經存在，`docker compose up` 將會嘗試停止容器，然後重新建立（保持使用 `volumes-from` 掛載的卷），以保證新啟動的服務對應 Compose 檔案的最新內容。如果使用者不希望容器被停止並重新創建，可以使用 `docker compose up --no-recreate`。這樣將只會啟動處於停止狀態的容器，而忽略已經執行的服務。如果使用者只想重新部署某個服務，可以使用 `docker compose up --no-deps -d <SERVICE_NAME>` 來重新建立服務並後台停止舊服務，啟動新服務，並不會影響到其所依賴的服務。
