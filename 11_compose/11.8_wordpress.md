...
WordPress 是全球最流行的內容管理系統 (CMS)。使用 Docker Compose 可以在幾分鐘內搭建一個包含資料庫、Web 服務和持久化儲存的本地練習/單機示範環境。
...
```yaml
  backup:
    ...
    volumes:
      - ./backups:/backup
    environment:
      # tiredofit/db-backup 4.x 起按 DB01_ 字首設定備份任務，並原生支援 _FILE 讀密
      - DB01_TYPE=mysql
      - DB01_HOST=db
      - DB01_NAME=wordpress
      - DB01_USER=wordpress
...
```yaml
      # 官方 wordpress 映象只把固定的一組 WORDPRESS_* 變數寫進 wp-config.php，
      # 其中並沒有 Redis 相關項；Redis Object Cache 外掛讀的是 PHP 常數 WP_REDIS_HOST，
      # 因此要透過 WORDPRESS_CONFIG_EXTRA 注入
