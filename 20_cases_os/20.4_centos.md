...
`CentOS` 和 `Fedora` 都是基於 `Redhat` 的常見 Linux 分支。傳統 `CentOS Linux` 曾是企業級伺服器的常用作業系統，但相關 Docker 官方映象已停止維護；新部署通常應評估 Rocky Linux、AlmaLinux、CentOS Stream 或直接使用 RHEL/UBI 等替代方案。`Fedora` 則主要面向個人桌面使用者。
