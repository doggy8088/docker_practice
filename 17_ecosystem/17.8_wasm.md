...
> **版本說明**：WebAssembly 和相關的 Wasm 執行時（如 WasmEdge、Spin）處於快速演進階段。建議查閱 [WebAssembly 官方文件](https://webassembly.org/)、[WasmEdge](https://wasmedge.org/) 和 [Spin](https://spinframework.dev/) 官方文件獲取最新訊息。
...
在現代的容器生態系統中，Wasm 並不被看作是要取代傳統 Docker 或者 Kubernetes 的技術，而是成為了一種 **與 Linux 容器互補並且共生** 的全新工作負載型別。
...
2. **透過容器執行時直接執行**：Docker 已經與如 [WasmEdge](https://wasmedge.org/) 和 [Spin](https://spinframework.dev/) 等高效能的企業級 Wasm 執行時進行了官方整合合作。
