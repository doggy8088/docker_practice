...
- containerd 2.0+ 移除了已棄用的 CRI v1alpha2 介面，僅保留 CRI v1（Kubernetes 1.26+ 僅支援 CRI v1）。如果叢集中仍有依賴 CRI v1alpha2 的元件，升級 containerd 2.x 前需先完成遷移。containerd 2.x 系列中 2.0 和 2.3 均為 LTS 版本，其中 2.3 支援從 1.7 LTS 直接升級，生產環境推薦使用。詳見 [containerd 發布說明](https://github.com/containerd/containerd/releases)。
