...
我們都知道，作業系統分為 **核心** 和 **使用者空間**。對於 `Linux` 而言，核心啟動後，會掛載 `root` 檔案系統為其提供使用者空間支援。而 **Docker 映象**（`Image`），就相當於是一個 `root` 檔案系統。比如官方映象 `ubuntu:24.04` 就包含了完整的一套 Ubuntu 24.04 最小系統的 `root` 檔案系統。
