## 3.7 macOS

### Mac 使用者的特殊考慮

macOS 上沒有原生 Linux 核心，Docker 需要執行在一個輕量級虛擬機中。Docker Desktop 完全封裝了這個複雜性，讓 Mac 使用者可以像 Linux 使用者一樣使用 Docker。但有幾點需要特別瞭解：

**效能屬性**：
- Apple Silicon（M 系列晶片）比 Intel Mac 的效能更好，且擁有原生支援
- 檔案 I/O 效能：macOS 與容器之間的卷掛載效能不如 Linux（這是虛擬化的代價）
- 記憶體使用：Docker Desktop 本身會消耗一定記憶體用於虛擬機管理

**許可考慮**：
- 個人開發和小團隊可免費使用
- 商業企業需要關注許可費用

### 3.7.1 系統要求

[Docker Desktop for Mac](https://docs.docker.com/desktop/setup/install/mac-install/) 支援當前版本及前兩個主要版本的 macOS，並且至少需要 4 GB 記憶體。對於 Apple Silicon 機型，若需要相容部分 Intel 指令行工具，官方建議安裝 Rosetta 2。

### 3.7.2 安裝

> [!WARNING]
> **商業許可限制**：自 2021 年起，Docker Desktop 對微型企業（少於 250 名員工且年收入少於 1000 萬美元）、個人使用、教育和非商業開源專案仍然免費。對於其他商業用途，需要付費訂閱。企業使用者請注意合規風險，或考慮使用開源替代方案。

Docker Desktop 為 Mac 使用者提供了無縫的 Docker 體驗。你可以選擇使用 Homebrew 或手動下載安裝套件進行安裝。

#### 使用 Homebrew 安裝

[Homebrew](https://brew.sh/) 的 [Cask](https://github.com/Homebrew/homebrew-cask) 已經支援 Docker Desktop for Mac，因此可以很方便的使用 Homebrew Cask 來進行安裝：

```bash
$ brew install --cask docker
```

#### 手動下載安裝

如果需要手動下載，請點選以下[連結](https://desktop.docker.com/mac/main/amd64/Docker.dmg)下載 Docker Desktop for Mac。

> 如果你的電腦搭載的是 Apple Silicon 晶片 (`arm64` 架構)，請點選以下[連結](https://desktop.docker.com/mac/main/arm64/Docker.dmg)下載 Docker Desktop for Mac。

如同 macOS 其它軟體一樣，安裝也非常簡單，雙擊下載的 `.dmg` 檔案，然後將那隻叫 [Moby](https://www.docker.com/blog/call-me-moby-dock/) 的鯨魚圖示拖拽到 `Application` 資料夾即可 (其間需要輸入使用者密碼)。

![圖](../_images/install-mac-dmg.jpg)

### 3.7.3 執行

從應用中找到 Docker 圖示並點選執行。

![圖](../_images/install-mac-apps.jpg)

執行之後，會在右上角選單欄看到多了一個鯨魚圖示，這個圖示表明了 Docker 的執行狀態。

![圖](../_images/install-mac-menubar.png)

每次點選鯨魚圖示會彈出操作選單。

![圖](../_images/install-mac-menu.png)

之後，你可以在終端透過指令檢查安裝後的 Docker 版本。

```bash
$ docker version
$ docker info
```
如果 `docker version` 和 `docker info` 都能正常回傳訊息，就可以嘗試執行一個 [Nginx 伺服器](https://hub.docker.com/_/nginx/)：

```bash
$ docker run -d -p 80:80 --name webserver nginx
```
服務執行後，可以訪問 [http://localhost](http://localhost)，如果看到了 『Welcome to nginx！』，就說明 Docker Desktop for Mac 安裝成功了。

![圖](../_images/install-mac-example-nginx.png)

要停止 Nginx 伺服器並刪除執行下面的指令：

```bash
$ docker stop webserver
$ docker rm webserver
```

### 3.7.4 映象加速

如果在使用過程中發現拉取 Docker 映象十分緩慢，可以設定 Docker [國內映象加速](3.9_mirror.md)。
