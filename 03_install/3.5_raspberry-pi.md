...
**映象可用性**：
- 並非所有 Docker 映象都有 ARM 版本（arm64 或 armv7）
- 官方映象通常提供多架構支援，但第三方映象可能沒有
- 某些依賴 Intel 特定指令的應用無法在 ARM 上執行

**儲存和記憶體**：
- 容器映象會佔用較多儲存空間，128GB microSD 卡建議最多執行 3-4 個中等大小的容器
...
Docker 官方目前單獨提供的是 **Raspberry Pi OS 32-bit (armhf)** 安裝頁，支援情況如下：

* Raspberry Pi OS Bookworm
* Raspberry Pi OS Bullseye

> **重要提醒**：Docker Engine v28 是官方最後一個支援 Raspberry Pi OS 32-bit (armhf) 的大版本。從 v29 開始，32 位 Raspberry Pi OS 不再提供新主版本軟體套件。若你使用的是 64 位 Raspberry Pi OS，請直接參考 Debian `arm64` 安裝方式。
...
在 `Raspberry Pi OS Bullseye/Bookworm` 中，如果你使用 `add-apt-repository` 新增源，可能會出現如下報錯。官方當前更推薦的做法是**不要繼續回退到舊式單行 `deb` 設定**，而是直接使用上面的 `docker.sources` 檔案方式寫入倉庫：
