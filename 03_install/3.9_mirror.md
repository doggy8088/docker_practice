...
本節以 [毫秒映象](https://docker.1ms.run) 映象服務 `https://docker.1ms.run` 為例進行介紹。由於映象加速服務經常變動，建議查閱上方的映象測試倉庫獲取最新可用地址。
...
對於使用 `Windows 10/11` 的使用者，在工具列托盤 Docker 圖示內右鍵選單選擇 `Change settings`，開啟設定視窗後在左側導向選單選擇 `Docker Engine`，在右側像下邊一樣編輯 json 檔案，之後點選 `Apply & Restart` 儲存後 Docker 就會重啟並應用設定的映象地址了。
...
可以登入[阿里雲容器映象服務](https://www.aliyun.com/product/acr?source=5176.11533457&userCode=8lx5zmtu&type=copy)**映象中心**->**映象搜尋** 查詢。
