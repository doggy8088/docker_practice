...
> [!NOTE]
> Replication Controller 保證指定數量的 Pod 副本在任何時候都處於執行狀態。

### 服務

> [!NOTE]
> 服務（Service）定義一組 Pod 的邏輯集合和訪問它們的策略。

### 卷

> [!NOTE]
> 卷（Volume）包含可被 Pod 中容器訪問的資料的目錄。

### 標籤

> [!NOTE]
> 標籤（Label）是附加到物件（如 Pods）上的鍵值對，用於組織和選擇物件子集。

### 介面許可權

> [!NOTE]
> 介面許可權透過認證、授權和准入控制來保護 Kubernetes API 的訪問。

### web介面

> [!NOTE]
> Kubernetes Dashboard 是一個基於 Web 的使用者介面，用於管理叢集。

### 指令行操作

> [!NOTE]
> kubectl 是 Kubernetes 的指令行工具，用於與叢集進行互動。
