...
Kubernetes 自 v1.35 起棄用 cgroup v1，預設要求節點使用 cgroup v2。從 v1.35 開始，kubelet 在 cgroup v1 節點上預設會拒絕啟動，但管理員可以在 kubelet 設定中設定 `failCgroupV1: false` 來相容 cgroup v1（僅建議用於遺留系統轉場期）。驗證節點是否支援 cgroup v2：
...
所有服務啟動後，透過 `crictl` 檢視本地實際執行的容器。這些服務大概分為三類：主節點服務、工作節點服務和其他服務。
