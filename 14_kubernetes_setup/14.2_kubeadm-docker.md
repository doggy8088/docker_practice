...
> ⚠️ **重要說明**：自 Kubernetes 1.24 起，內建 `dockershim` 已被移除，Kubernetes 預設不再直接使用 Docker Engine 作為容器執行時 (CRI)。因此，**更推薦參考** 同目錄下的《[使用 kubeadm 部署 Kubernetes (CRI 使用 containerd)](14.1_kubeadm.md)》。
>
