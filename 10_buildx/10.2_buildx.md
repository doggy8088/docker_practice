...
> 2. **啟用 containerd image store**：在 Docker 守護程序中啟用 `containerd image store` 屬性（Docker v29+，新安裝場景預設啟用，Docker Desktop 上也更容易直接使用），可以在本地檢視和管理 SBOM 等 attestation 元資料。

### 10.2.2 官方文件

* [Docker buildx 指令文件](https://docs.docker.com/engine/reference/commandline/buildx/)
