...
節點的狀態透過一組條件（Conditions）來描述。主要條件包括 `Ready`（kubelet 健康且可以接收 Pod）、`MemoryPressure`（記憶體不足）、`DiskPressure`（磁碟不足）和 `PIDPressure`（程序數過多）等。其中 `Ready` 條件最為關鍵：值為 `True` 表示節點健康可調度，`False` 表示節點異常，`Unknown` 表示節點控制器超過一定時間未收到心跳。
