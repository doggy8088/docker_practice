...
* `apiserver` 是整個系統的對外介面，提供一套 RESTful 的 [Kubernetes API](https://kubernetes.io/zh-cn/docs/concepts/overview/kubernetes-api/)，供用戶端和其他元件呼叫；
* `scheduler` 負責對資源進行排程，分配某個 pod 到某個節點上。是 pluggable 的，意味著很容易選擇其他實現方式；
