...
配套 demo 中的 [`ssl/README.md`](demo/ssl/README.md) 只保留本地生成證書的佔位說明，真實私鑰和證書不應送出到倉庫。
...
> **安全提示**：這些私鑰和證書應在本地或受控部署環境中生成，不要送出進 Git 倉庫。範例目錄只保留佔位說明，執行前請按上面的步驟重新生成。
...
>
> **安全提示**：上述指令會將密碼明文暴露在 shell 歷史記錄和程序清單中。生產環境建議使用互動式方式輸入密碼（不帶 `-b` 引數），或透過環境變數/檔案傳入。
> 配套 demo 的 [`auth/README.md`](demo/auth/README.md) 僅說明本地生成步驟，生成的 `auth/nginx.htpasswd` 已被忽略，不應送出。

> **版本說明**：使用 `httpd:2.4-alpine` 基於 Apache 2.4 的精簡映象。如需其他版本，請指定具體版本號如 `httpd:2.4.58-alpine`；不要用 `latest`（本節下文與 7 章都要求固定版本以保證可重複性）。

### 6.3.4 編輯 Docker Compose 設定
...
本書配套的 `06_repository/demo/` 也採用同樣約定：容器內 registry 監聽 `:5000`，宿主機透過 `443:5000` 暴露 HTTPS 服務。這樣可以避免在容器內佔用特權連接埠，同時仍讓用戶端使用 `https://docker.domain.com` 訪問。
...
> **版本說明**：範例使用 `ubuntu:24.04`，這是仍受 Docker 支援的 LTS 版本（當前最新 LTS 為 26.04）。推送到私有倉庫時保持了原有的版本標籤，建議生產環境明確指定映象版本號。
如果我們退出登入，嘗試推送映象。
...
如果你本機佔用了 `443` 連接埠，你可以設定 [Nginx 代理](https://distribution.github.io/distribution/recipes/nginx/)，這裡不再贅述。
