...
[Docker Registry](https://distribution.github.io/distribution/) 是官方提供的工具，可以用於建立私有的映象倉庫。本文範例沿用 [distribution/distribution](https://github.com/distribution/distribution) v2.x 相容路徑；新生產部署應評估 Distribution 3.x，並核對設定路徑、遷移說明和生態相容性。

### 6.2.1 安裝執行 docker-registry

#### 容器執行

如果您需要搭建私有倉庫，可以透過官方提供的 `registry` 映象快速部署。
...
> **注意**：這樣啟動的 registry **沒有任何認證、也沒有 TLS**，因此這裡刻意繫結到
> `127.0.0.1`，僅供本機實驗。不要直接把它綁到 `0.0.0.0` 或內網地址——那等於允許同網段
> 任意主機匿名推送映象（映象投毒：覆蓋你的 `latest` 標籤，下游全部拉到攻擊者的映象）。
> 需要多機訪問請直接用 [6.3 節](6.3_registry_auth.md) 的 TLS + 認證方案。
...
> **警告**：下面的 `insecure-registries` 會關閉該地址的 TLS 校驗，推送和拉取都是明文、可被中間人篡改；再疊加上 registry 預設無認證，等於把「任意主機可匿名投毒」的倉庫開在內網。**僅限隔離的實驗網路**，生產一律走 [6.3 節](6.3_registry_auth.md) 的 TLS + 認證。

#### Linux

預設情況下，Docker 強制使用 HTTPS 協定推送映象。如果您搭建的私有倉庫是 HTTP 協定，需要進行如下設定。
...
```
> 注意：該檔案必須符合 `json` 規範，否則 Docker 將不能啟動。映象加速器地址請替換為實際可用的源，具體設定參見 [3.9 映象加速器](../03_install/3.9_mirror.md)。
