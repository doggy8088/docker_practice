...
Docker 的儲存驅動經歷了從早期各式各樣的機制（如 aufs, devicemapper），到被廣泛使用的現代經典 graph driver (`overlay2`)，再到當下（Engine v29 及以後）**預設啟用的 containerd 映象儲存引擎（containerd image store）**的演進。

| 儲存後端 / 驅動 | 核心屬性說明 | 推薦程度 |
|---------|------|---------|
| **containerd image store** | (v29+ 新一代預設引擎) 基於 containerd 的 snapshotters，原生支援 OCI image index、多架構映象與 Attestations 建立溯源元資料儲存。 | ✅**強烈推薦 (現代預設)** |
...
```bash
## 檢視預設儲存驅動 (Storage Driver)
$ docker info | grep "Storage Driver"
Storage Driver: overlay2

## 在 Engine v29+ 中，可以透過如下輸出驗證是否開啟了 containerd 映象後端：
