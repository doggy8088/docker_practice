...
Docker 20.10+ 開始支援 cgroups v2（如果系統支援），提供更好的效能和資源隔離。cgroup 驅動可以透過 Docker 守護程序設定檔案 `/etc/docker/daemon.json` 的 `exec-opts` 設定項指定：
...
`native.cgroupdriver` 僅支援 `systemd` 和 `cgroupfs` 兩個取值；若不指定，cgroup v1 主機預設使用 `cgroupfs`，帶 systemd 的 cgroup v2 主機預設使用 `systemd`。重啟 Docker 守護程序後生效。
