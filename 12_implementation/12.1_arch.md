...
### 12.1.4 Docker Engine v29+ 變化

從 Docker Engine v29 開始，架構進一步簡化和標準化：

- **Containerd 映象儲存 (Image Store)**：在 v29+ 的新安裝場景中預設啟用。Docker 直接使用 Containerd 的映象管理能力，不再維護自己的一套 graphdriver。
...
- [命名空間](./12.2_namespace.md)：Runc 如何隔離容器
- [控制組](./12.3_cgroups.md)：Runc 如何限制資源
- [聯合檔案系統](./12.4_ufs.md)：映象如何儲存
