...
在現代 Docker 版本中，你可以使用[多階段建立](../07_dockerfile/7.17_multistage_builds.md)來減少所建立映象的大小。該能力最早在 Docker 17.05 引入，如今已是編寫生產映象的預設實踐之一；新專案還應結合 BuildKit 快取掛載、secret 掛載和多平臺建立能力一起評估。
...
* 對於 `ADD` 和 `COPY` 指令，映象中對應檔案的內容也會被檢查，每個檔案都會計算出一個校驗和。檔案的最後修改時間和最後訪問時間不會納入校驗。在快取的查詢過程中，會將這些校驗和與已存在映象中的檔案校驗和進行對比。如果檔案有任何改變，比如內容和元資料，則快取失效。
...
```
關於標籤可以接受的鍵值對，參考 [Label keys and values](https://docs.docker.com/engine/manage-resources/labels/#label-keys-and-values)。關於查詢標籤訊息，參考 [Manage labels on objects](https://docs.docker.com/engine/manage-resources/labels/#manage-labels-on-objects)。
...
> 注意：exec 形式是 JSON 陣列，**必須用雙引號**。寫成 `CMD ['apache2', '-DFOREGROUND']` 並不會報錯，但 Docker 解析不出 JSON，會退回 shell 形式執行整串字元，導致啟動失敗。參見 [7.4 CMD](../07_dockerfile/7.4_cmd.md)。
...
這些官方映象的 Dockerfile 都是參考典範，詳見 [docker-library/docs](https://github.com/docker-library/docs)。
