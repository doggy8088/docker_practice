...
| `permission denied while trying to connect to the Docker daemon socket` | 當前使用者無權訪問 Docker socket | 臨時使用 `sudo docker ...`，或確認風險後執行 `sudo usermod -aG docker $USER` 並重新登入；`docker` 使用者組等同授予 root 級許可權，安全要求較高的環境優先評估 Rootless mode |
| `manifest for ... not found: manifest unknown` | 映象 tag 不存在 | 檢查 Docker Hub 該映象是否存在該 tag，或拼寫是否正確 |
| `connection refused` (pull image) | 網路不通或映象源無法訪問 | 檢查網路，設定[映象加速器](../../03_install/3.9_mirror.md) |
