...
許多生產環境一般是沒有公網資源的，這就需要從有公網資源的伺服器上把映象匯出，然後匯入到需要執行映象的內網伺服器。
...
```
> 使用 docker save 的時候，也可以使用 image id 來匯出，但是那樣匯出的時候，就會丟失原來的映象名稱，推薦，還是使用映象名字+tag 來匯出映象
...
開啟 `http://<server-ip>:9090` 訪問 Web 控制台。
