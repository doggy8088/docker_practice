...
# 安裝額外工具
## Alpine 只提供帶主版本號的 contrib 套件（postgresql16-contrib 等），
## 不存在 postgresql-contrib；pg-stat-monitor 也未打進 Alpine 倉庫，需要另行編譯
...
> 下面範例聚焦服務拓撲、網路、健康檢查和資源限制。為便於閱讀，密碼仍透過環境變數串接；生產環境應改用 Compose secrets、外部金鑰系統或雲廠商金鑰服務，並避免把敏感值寫入 `DATABASE_URL` / `REDIS_URL`。
