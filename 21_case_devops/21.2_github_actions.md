...
倉庫還提供一份經 `actionlint` 校驗、並將 `checkout` 固定到完整 commit SHA 的[可執行工作流範例](../examples/validated/github-actions/validate.yml)。為了讓正文突出結構，下面較長的教學片段仍用主版本 tag；生產工作流應採用完整 SHA。
...
* `provenance: mode=max` 和 `sbom: true` 會把建立來源證明和 SBOM 附加到推送的映象上；只做本地 `load: true` 的建立無法完整保留這些 attestation。
