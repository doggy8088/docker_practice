...
flowchart TD
    subgraph NormalProcess ["普通程序"]
        direction TB
        N1["• 與其他程序共享系統資源<br/>• 可以看到其他程序<br/>• 共享網路和檔案系統"]
    end
    
    subgraph ContainerProcess ["容器程序 (執行在宿主機核心上)"]
        direction TB
        C1["• 有自己的程序空間（看不到宿主機上的其他程序）<br/>• 有自己的網路（獨立 IP、連接埠）<br/>• 有自己的檔案系統（獨立的 root 目錄）<br/>• 有自己的使用者（容器內的 root ≠ 宿主機的 root）"]
    end
```

這種隔離是透過 Linux 核心的 **Namespace** 技術實現的。

### 2.2.3 容器 vs 虛擬機：核心區別
...
flowchart TD
    ...
    ImageLayerN["映象第 N 層（只讀）"]
    ImageLayerN1["映象第 N-1 層（只讀）"]
    Dots["..."]
    ImageLayer1["映象第 1 層（只讀）<br/>基礎映象層"]
...
掌握容器的生命週期對於管理和除錯 Docker 應用非常重要。如圖 2-1 所示，容器會經歷從建立到刪除的完整狀態流轉。
...
> 想深入瞭解？請閱讀[底層實現 - 命名空間](../12_implementation/12.2_namespace.md)。
