...
> **容器是映象的執行實例。如果把映象比作程式，那麼容器就是程序。**

用物件導向程式設計的術語來說：**映象是類（Class），容器是物件（Instance）**。
...
| 方式 | 說明 | 適用場景 |
|------|------|---------|
| **[資料卷（Volume）](../07_data_network/data/volume.md)** | Docker 管理的儲存 | 資料庫、應用資料 |
| **[繫結掛載（Bind Mount）](../07_data_network/data/bind-mounts.md)** | 掛載宿主機目錄 | 開發時共享程式碼 |
...
詳細解釋請參考[後台執行](../05_container/daemon.md)章節。
...
> 想深入瞭解？請閱讀[底層實現 - 命名空間](../13_implementation/namespace.md)。
...
理解了映象和容器，接下來讓我們學習[倉庫](repository.md)——儲存和分發映象的服務。

## 延伸閱讀

- [啟動容器](../05_container/run.md)：詳細的容器啟動選項
- [後台執行](../05_container/daemon.md)：理解容器為什麼會"立即退出"
- [進入容器](../05_container/attach_exec.md)：如何操作執行中的容器
