...
> **版本說明**：本節範例基於 Docker v29.x 編寫。範例中使用的映象標籤（如 `ubuntu:24.04`、`nginx:latest`）為示範用途，建議查閱 [Docker Hub](https://hub.docker.com) 或相應映象的官方頁面確認最新可用版本。
...
flowchart TD
    ...
    Layer2["第 2 層: nginx 安裝檔案與套件索引 (只讀)"]
...
...
```
需要注意：`docker history` 展示的是映象的建立歷史。像 `CMD`、`ENTRYPOINT`、`EXPOSE`、`STOPSIGNAL` 這類 `0B` 項主要是映象設定元資料，並不等同於新增了可見的檔案系統內容；真正佔用空間的通常是 `RUN`、`COPY`、`ADD` 等帶來的檔案變化。
...
#### 2. 映象 ID：Content-Addressable 標識
