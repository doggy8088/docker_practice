...
> **Docker 映象是一個只讀的樣板，包含了執行應用所需的一切：程式碼、執行時、函式庫、環境變數和設定檔案。**

如果用一個類比：**映象就像是一張光碟或 ISO 檔案**。你可以用同一張光碟在不同電腦上安裝系統，而光碟本身不會被修改。同樣，一個映象可以建立多個容器，而映象本身保持不變。
...
```
┌─────────────────────────────────────┐
│  第 4 層: COPY app.conf (只讀)       │  ← 最新新增的層
├─────────────────────────────────────┤
│  第 3 層: nginx 安裝檔案 (只讀)       │
├─────────────────────────────────────┤
│  第 2 層: apt 快取更新 (只讀)         │
├─────────────────────────────────────┤
│  第 1 層: Ubuntu 基礎系統 (只讀)      │  ← 基礎映象層
└─────────────────────────────────────┘
```
...
> ⚠️ **筆者特別提醒**：理解這一點可以幫你避免建立出臃腫的映象。

**關鍵原理**：每一層的檔案變化會被記錄，但**刪除操作只是標記，不會真正減小映象體積**。
...
理解了映象，接下來讓我們學習[容器](container.md)——映象的執行實例。

## 延伸閱讀

- [獲取映象](../04_image/pull.md)：從 Registry 下載映象
- [使用 Dockerfile 定製映象](../04_image/build.md)：建立自己的映象
- [Dockerfile 最佳實踐](../15_appendix/best_practices.md)：建立高質量映象的技巧
- [底層實現 - 聯合檔案系統](../13_implementation/ufs.md)：深入理解分層儲存的技術原理
