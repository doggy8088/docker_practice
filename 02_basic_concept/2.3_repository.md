...
> **版本說明**：本節範例基於 Docker v29.x 和常見映象版本編寫。範例中的版本號（如 `nginx:1.28`、`mysql:8.4`、`mysql:5.7` 等）為示範用途。實際使用時請訪問 [Docker Hub 官方頁面](https://hub.docker.com) 或相應映象的發布頁確認最新可用版本和標籤。
...
當前更推薦使用 Sigstore / Notation 體系進行映象簽名與驗證。`Docker Content Trust (DCT)` 已被正式退役：自 Docker Engine 29.0 起 DCT 已從 Docker CLI 中移除（`docker trust` 子指令不再隨 CLI 發布，僅能作為獨立外掛自行建立；`docker push`、`docker pull`、`docker build`、`docker run` 等指令上的 `--disable-content-trust` 選項雖仍可解析，但已標記為廢棄且不再有實際作用），Docker 官方的 Notary v1 服務 `notary.docker.io` 也將於 2026 年 12 月 8 日完全關閉；最早一批 DCT 簽名證書自 2025 年 8 月 8 日起已開始過期，Docker 建議映象發布者遷移到 Sigstore、Notation 等方案。不建議把 DCT 作為新專案方案。
