...
當前更推薦使用 Sigstore / Notation 體系進行映象簽名與驗證。`Docker Content Trust (DCT)` 已進入棄用階段：Docker 官方說明最早一批 DCT 簽名證書自 2025 年 8 月 8 日起開始過期，並建議映象發布者遷移到 Sigstore、Notation 等方案；完整棄用時間線仍以 Docker 後續公告為準。不建議把 DCT 作為新專案方案。
