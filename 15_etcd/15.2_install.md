...
> 注意：etcd 3.7.0 於 2026 年 7 月發布，官方仍在維護 3.5、3.6、3.7 三個發布分支。etcd 3.4 已隨 2026 年 6 月的 v3.4.45 結束支援（EOL），仍在使用的使用者應儘快升級。本章範例基於 etcd `3.5.x` 版本編寫，etcd 3.6.x/3.7.x 可用於新部署。請訪問 [etcd 官方發布頁](https://github.com/etcd-io/etcd/releases) 獲取最新版本。
...
```bash
# 下載 etcd v3.5.33 版本（請訪問 https://github.com/etcd-io/etcd/releases 獲取最新版本）
...
映象名稱為 `quay.io/coreos/etcd`，可以透過下面的指令啟動單機實驗用 `etcd` 服務，並只把 `2379` 和 `2380` 對映到宿主機回環地址。

> **版本說明：** 範例中使用 `v3.5.33` 標籤。請訪問 [etcd 官方發布頁](https://github.com/etcd-io/etcd/releases) 獲取最新可用版本標籤。
...
上面範例僅用於單機實驗：宿主機連接埠限定在 `127.0.0.1`，因此不會把未啟用 TLS 的 etcd 直接暴露到外部網路。容器內部的 `listen-*` 可以繫結 `0.0.0.0` 監聽容器網絡卡，但 `advertise-*` 應填寫其他節點或用戶端**實際可訪問的主機地址**，不能直接寫成 `0.0.0.0`。生產或多節點環境不要使用明文 HTTP 暴露 etcd，應結合固定內網地址、TLS、用戶端證書認證和防火牆訪問控制。
