...
| 區塊 I/O | `BLOCK I/O`、`container_fs_*` / `container_blkio_*` | `io.stat` | `blkio.throttle.io_service_bytes` |
...
# 最終映象大小通常 < 15MB（相比 golang:1.26 基礎映象的 ~900MB）
```
**Node.js 應用的多階段建立：**
...
COPY requirements.txt .
## 裝到共享字首而不是 /root/.local：/root 許可權是 0700，
## 執行階段切到 USER nobody 後將無法讀取，容器會起不來
