...
> `"ENV key=value" should be used instead of legacy "ENV key value" format`，`docker buildx build --check`（見 [10.2](../10_buildx/10.2_buildx.md)）會直接把它標出來。一律用等號。
...
## ⚠️ 好一些但仍會洩漏：-e 會進入 shell history、宿主機程序 argv（同機任何使用者 ps 可見），
## 並永久固化在 docker inspect 的 Config.Env 裡

## docker run -e DB_PASSWORD=xxx myimage

## ✅ 正確：用檔案傳入，不進指令行

## docker run --env-file ./db.env myimage        # 檔案許可權設為 600
## 或使用 Compose secrets / *_FILE 約定，見 11.8 節
...
## ⚠️ 多個 ENV 會多出幾條建立歷史記錄（元資料指令，不增加檔案系統層）
