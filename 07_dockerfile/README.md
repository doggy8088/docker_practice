...
可執行 `docker buildx build --check` 校驗的[完整 Dockerfile 範例](../examples/validated/dockerfile/Dockerfile)與配套 `index.html` 儲存在倉庫中。它使用 `scratch`，因此結構檢查不會解析遠端基礎映象元資料。下面的圍欄與該檔案由測試強制保持一致。
...
- **基礎映象版本號**（如 `node:22`、`python:3.12`）：使用主或次版本號而非完整版本號（patch），這樣可以自動獲取最新的補丁版本，確保獲得安全更新。
  （這條只針對 **Dockerfile 的基礎映象**；部署一個服務映象時反而應釘死確切版本，見 [第 5 章](../05_container/README.md)。）
