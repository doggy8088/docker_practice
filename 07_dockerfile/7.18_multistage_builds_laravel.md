...
```docker
# 注：node 映象推薦使用具體的版本標籤（如 node:20-alpine）
...
```docker
# 注：nginx 映象推薦使用具體的版本標籤（如 nginx:1.25-alpine）
...
```docker
# 注：生產環境推薦使用具體的版本標籤，如 node:20-alpine、composer:2.x、php:8.3-fpm-alpine、nginx:1.25-alpine
FROM node:alpine as frontend
