...
`docker commit` 除了幫助理解映象分層之外，在少數場景下也可用於留存現場，例如事後分析被入侵容器的狀態；但是，日常定製映象不應依賴 `docker commit`，而應使用下一節介紹的 `Dockerfile`。
...
回顧一下之前我們學到的知識，映象是多層儲存，每一層是在前一層的基礎上進行的修改；而容器同樣也是多層儲存，它以映象層為只讀基礎，並在最上方增加一層供執行時寫入的容器層。

現在讓我們以定製一個 Web 伺服器為例子，來講解映象是如何建立的。

> **版本提示**：以下範例中 `nginx` 映象使用預設 `latest` 標籤。生產環境建議指定具體版本號（如 `nginx:1.27`），以避免映象更新帶來的不相容性。

```bash
$ docker run --name webserver -d -p 8080:80 nginx
```
這條指令會用 `nginx` 映象啟動一個容器，命名為 `webserver`。其中，`-p 8080:80` 表示把宿主機的 `8080` 連接埠對映到容器內的 `80` 連接埠，這樣我們就可以用瀏覽器去訪問這個 `nginx` 伺服器。

如果是在本機執行的 Docker，那麼可以直接訪問：`http://localhost:8080`；如果是在虛擬機、雲服務器上安裝的 Docker，則需要將 `localhost` 換為虛擬機地址或者實際雲服務器地址，並保留 `8080` 連接埠。
...
```
我們可以用下面的指令將容器儲存為映象。預設情況下，`docker commit` 會在送出時暫停容器程序，以降低資料損壞的風險；如果確實不希望暫停，可以顯式指定 `--no-pause`：
...
我們可以在 `docker image ls` 中看到這個新定製的映象。下面的輸出僅為範例，標籤、建立時間和大小會隨著映象版本和本地環境不同而變化：
...
> **版本說明**：上面範例中 `nginx:1.27` 代表 1.27 系列的最新 patch 版本。在實際應用中應根據需求選擇確切的版本號，而不是盲目使用 `latest`。

我們還可以用 `docker history` 具體檢視映象內的歷史記錄。例如先執行 `docker history nginx:v2`，再對比 `docker history nginx:latest`，就能看到我們剛剛送出出來的新層。
...
```
這裡我們將新容器命名為 `web2`，並把宿主機的 `81` 連接埠對映到容器的 `80` 連接埠。訪問 `http://localhost:81` 後，看到的內容應該和之前修改後的 `webserver` 一樣。

至此，我們第一次完成了定製映象，使用的是 `docker commit` 指令，手動操作給舊的映象新增了新的一層，形成新的映象，對映象多層儲存應該有了更直觀的感覺。

### 4.4.1 慎用 `docker commit`

使用 `docker commit` 指令雖然可以比較直觀地幫助理解映象分層儲存的概念，但它不應作為常規定製映象的方式。
...
而且，回顧之前提及的映象所使用的分層儲存的概念，除當前層外，之前的每一層都不會發生改變。換句話說，任何修改的結果僅僅是在當前層進行標記、新增、修改，而不會改動上一層。如果使用 `docker commit` 製作映象，以及後期繼續修改，那麼每一次修改都會讓映象再膨脹一層；即使某些檔案在更高層裡被刪除了，它們仍然存在於更低層中，只是在最終檢視裡被隱藏而已。因此，日常定製映象應使用下一節介紹的 `Dockerfile`，把建立過程寫成可重複執行、便於審查的文字。
