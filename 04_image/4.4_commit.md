...
> **版本提示**：以下範例中 `nginx` 映象使用預設 `latest` 標籤。生產環境建議指定具體版本號（如 `nginx:1.30`），以避免映象更新帶來的不相容性。
...
> **版本說明**：上面範例中 `nginx:1.30` 代表 1.30 系列的最新 patch 版本。在���際應用中應根據需求選擇確切的版本號，而不是盲目使用 `latest`。
