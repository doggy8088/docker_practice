...
除了標準的使用 `Dockerfile` 生成映象的方法外，由於各種特殊需求和歷史原因，還提供了一些其他方法用以生成映象。

### 4.6.1 從 rootfs 壓縮封裝匯入
...
> **版本提示**：`noble` 對應 Ubuntu 24.04 LTS。實際用於生產環境時，應選擇仍在安全維護期內的發行版，並按團隊的基礎映象更新策略定期重建。
...
### 4.6.2 Docker 映象的匯入和匯出 `docker save` 和 `docker load`
...
> **版本提示**：`alpine:latest` 為最新版本的 Alpine Linux。如果需要特定版本號（如 `alpine:3.24`），可以明確指定以確保可重現性；注意選一個仍在維護的版本，`alpine:3.20` 已於 2026-04 結束支援。
儲存映象的指令為：
...
```
然後我們將 `alpine-latest.tar.gz` 檔案複製到了另一個機器上，可以用下面這個指令載入映象：
