...
倉庫名和標籤都為 `<none>` 的映象被稱為虛懸映象。自 Docker Engine 29 起，`docker image ls` 預設不再列出它們，需要 `-a` 或 `dangling=true` 過濾器才能看到。
...
`docker image ls` 預設列出的只是有標籤的頂層映象。除了虛懸映象，被隱藏的還有一類為了加速映象建立、重複利用資源而存在的中間層映象。
...
$ docker images -a
```
會顯示預設隱藏的映象：建立過程中產生的中間層，以及上面提到的虛懸映象。中間層被其他映象依賴。
